...
export default async function FarmerView({ userId }: { userId: string }) {
  ...
  return (
    <div className="space-y-6">
      {/* 1. 顶部操作栏 */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 bg-white p-6 rounded-xl shadow-sm border border-green-100">
        <div>
          <h2 className="text-2xl font-bold text-green-900">我的种植管理</h2>
        ...
        {batches.length === 0 ? (
          ...
          batches.map((batch) => {
            ...
            return (
              <Card
                key={batch.id}
                className="group hover:border-green-400 transition-all duration-300 overflow-hidden"
              >
                {/* Header: 显示批次号和状态 */}
                <CardHeader className="bg-green-50/30 border-b border-green-100 py-3 flex flex-row items-center justify-between">
                ...
                <CardFooter className="bg-gray-50/50 py-3 gap-3">
                  <Link
                    ...
                    className="flex-1"
                  >
                    <Button className="w-full bg-green-600 hover:bg-green-700 h-9 shadow-sm">
                  ...
                  >
                    <Button
                      variant="outline"
                      className="w-full bg-white hover:bg-green-50 hover:text-green-700 h-9"
                    >
